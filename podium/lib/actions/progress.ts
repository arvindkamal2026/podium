...
export async function updatePIProgress(piId: string, isCorrect: boolean) {
  ...
  try {
    ...
    const db = getAdminDb();
    const ref = db.collection("users").doc(uid).collection("piProgress").doc(piId);
    ...
    if (!snap.exists) {
      ...
      if (isCorrect && newConsecutive >= 3) {
        newStatus = "mastered";
      } else if (!isCorrect && data.status === "mastered") {
        newStatus = "learning";
      } else if (data.status === "untested") {
        newStatus = "learning";
      }
    ...
    console.error("PI progress error:", error);
